...
public sealed record SyncPlanResponse(
    string Version,
...
public sealed record SyncResourcePull(
    string? Filter);

public sealed record SyncResourcePush(
    bool AllowCreate,
    bool AllowUpdate,
    bool AllowDelete);

public sealed record SyncPlanFilters(
    string? Global,
    IReadOnlyDictionary<string, string> ByEntity);

public sealed record SyncCursorPolicy(
    string Scope,
    bool ResetOnVersionChange);
